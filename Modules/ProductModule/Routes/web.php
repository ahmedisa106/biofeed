...
Route::group(['prefix' => 'admin-panel', 'middleware' => ['auth:admin']], function() {
    ...
    Route::prefix('/')->group(function () {
        ...
        Route::group(['middleware' => ['role:superadmin|owner']], function () {
            Route::resource('product', 'ProductController')->only(['destroy','show']);
            Route::delete('/delete/{id}', 'ProductCategoriesController@destroy');

            Route::delete('/product-photo/delete/{id}','ProductController@deletepic');
            Route::post('/product-photos','ProductController@storeAlbum');
        });
